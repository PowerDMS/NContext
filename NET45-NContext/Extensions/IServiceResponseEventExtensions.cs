namespace NContext.Extensions
{
    ...
    public static class IServiceResponseEventExtensions
    {
        ...
        public static IServiceResponse<T> Raise<T, TEvent>(this IServiceResponse<T> serviceResponse, TEvent @event, Boolean onRightOnly = true)
        {
        ...
        /// <param name="onRightOnly">Determines whether to raise the event in the case the serviceResponse is left.</param>
        public static IServiceResponse<T> Raise<T, TEvent>(this IServiceResponse<T> serviceResponse, Func<T, TEvent> eventFactory, Boolean onRightOnly = true)
        {
            if (onRightOnly && serviceResponse.IsLeft)
            {
                return serviceResponse;
            ...
            return EventManager.RaiseEvent<TEvent>(eventFactory(serviceResponse.Data))
                .ContinueWith(task => task.IsFaulted
                    ? new ErrorResponse<T>(task.Exception.ToError())
                    : serviceResponse)
                .Result;
